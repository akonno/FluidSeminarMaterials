...
pygame2で説明したように，マウスカーソルの位置は`mx, my = pygame.mouse.get_pos()`で得られるので，長方形の位置をそれに合わせて動かす．ここでは長方形の高さは固定し，横方向にのみ動かしてみる．
...
```{admonition} 演習課題（発展）
:class: exercise
1. 現在の障害物くぐりゲームでは，同じ障害物がローテーションして右から左へ流れてくるが，障害物が画面の左に抜けたら穴の位置を新たに決めて，障害物を作り直すようにせよ．
