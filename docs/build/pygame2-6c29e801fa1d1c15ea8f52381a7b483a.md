...
最初のプログラムで作成した、ウィンドウを開いて図形を描くプログラムの構造を確認する。
...
これで，青い円が左から右に動くはずである．画面に描く物体の位置を少しずつ変え、同じ処理を短い間隔で繰り返すことが、アニメーションの基本である。

### 状態を少しずつ変える

動く物体を扱うときは、位置や色など、次のフレームにも引き継ぐ値を「状態」として持つ。例えば`x1`が円の現在位置を表し、メインループを1回通るたびに`x1`を増やすことで、円が移動する。

物体が一つだけなら一組の変数でよい。複数の物体を扱う場合は、物体ごとの位置や色をリストに入れ、`for`文で順に描画できる。

```python
objects = [
    {"x": 100, "y": 100, "color": (255, 50, 50)},
    {"x": 150, "y": 130, "color": (50, 255, 50)},
]

for obj in objects:
    pygame.draw.circle(screen, obj["color"], (obj["x"], obj["y"]), 20)
```

ここでは単純化のため、物体の位置を直接変更している。速度、加速度、時間刻みを使った運動の計算は、[pygame-ceの運動・衝突](./pygame3)で扱う。
